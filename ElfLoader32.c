...
int fill_headers32(Elf32_Ehdr *headers, const char *filename);
int get_pht_entries32(Elf32_Phdr ***entries, Elf32_Ehdr *headers, char *filename);
int get_sht_entries32(Elf32_Shdr ***entries, Elf32_Ehdr *headers, char *filename);
int get_sht_dynsym_index(Elf32_Shdr **entries, int n);
int loader32(Elf32_Phdr **p_entries, Elf32_Shdr **s_entries, Elf32_Ehdr *headers, char *filename);
int arch_identify(char *filename);
int valid_elf(FILE *file);
int main32(char *filename);
int file_exists (char *filename);
void relocate(Elf32_Shdr *shdr, Elf32_Sym *syms, char *strings, FILE *file, void *allocated_mem);
void *resolve_sym(const char* sym);
int find_symbol_table(Elf32_Ehdr *headers, Elf32_Shdr **s_entries);
void* find_sym(const char *sym_name, Elf32_Shdr **s_entries, Elf32_Shdr *symtab, FILE *file, void *allocated_mem);
...
int main32(char *filename) {
    int flag = 0;
    Elf32_Ehdr headers = {0};
    if(fill_headers32(&headers, filename)) {
        fprintf(stderr, "[x] fill_headers32 Failed.\n");
        return 1;
    }

    Elf32_Phdr **p_entries = NULL;
    Elf32_Shdr **s_entries = NULL;

    if (get_pht_entries32(&p_entries, &headers, filename)) {
        fprintf(stderr, "[x] get_pht_entries32 Failed\n");
        return 1;
    }

    if (get_sht_entries32(&s_entries, &headers, filename)) {
        fprintf(stderr, "[x] get_sht_entries32 Failed\n");
        return 1;
    }


    if(loader32(p_entries, s_entries, &headers, filename)) {
...
int fill_headers32(Elf32_Ehdr *headers, const char *filename) {
    ...
    // Read the ELF header
    size_t bytesRead = fread(headers, 1, sizeof(Elf32_Ehdr), file);
...
int get_pht_entries32(Elf32_Phdr ***entries, Elf32_Ehdr *headers, char *filename) {
    ...
    // Allocate memory for array of pointers to Elf32_Phdr
    *entries = malloc(headers->e_phnum * sizeof(Elf32_Phdr *));
    if (!*entries) {
...
int get_sht_entries32(Elf32_Shdr ***entries, Elf32_Ehdr *headers, char *filename) {
    ...
    // Allocate memory for array of pointers to Elf32_Phdr
    *entries = malloc(headers->e_shnum * sizeof(Elf32_Shdr *));
    if (!*entries) {
...
int loader32(Elf32_Phdr **p_entries, Elf32_Shdr **s_entries, Elf32_Ehdr *headers, char *filename) {
    ...
    for (int i = 0; i < headers->e_phnum; ++i) {
        if (p_entries[i]->p_type == PT_LOAD) {
            Elf32_Addr end_addr = p_entries[i]->p_vaddr + p_entries[i]->p_memsz;
    ...
    if (sht_dynsym_index != -1) {
        ...
        Elf32_Sym *global_symbol_table = malloc(s_entries[sht_dynsym_index]->sh_size);
...
void relocate(Elf32_Shdr *shdr, Elf32_Sym *syms, char *strings, FILE *file, void *allocated_mem) {
    ...
    Elf32_Rel *rel = malloc(shdr->sh_size);
    if (!rel) {
    ...
    for (int j = 0; j < shdr->sh_size / sizeof(Elf32_Rel); j++) {
        ...
        switch (ELF32_R_TYPE(rel[j].r_info)) {
            case R_386_JMP_SLOT:
            case R_386_GLOB_DAT:
                *(Elf32_Word *)(allocated_mem + rel[j].r_offset) = (Elf32_Word)(uintptr_t)resolve_sym(sym);
                break;
        }
    }
...
void* find_sym(const char *sym_name, Elf32_Shdr **s_entries, Elf32_Shdr *symtab, FILE *file, void *allocated_mem) {
    // Allocate memory for the symbol table
    Elf32_Sym *sym_table = malloc(symtab->sh_size);
    if (!sym_table) {
    ...
    Elf32_Shdr *strtab = s_entries[symtab->sh_link];
